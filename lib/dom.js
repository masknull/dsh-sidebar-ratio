...
(function () {
  ...
  function pxOf(track, fallback) {
    ...
    var s = track.trim()
    // DSH 0.1.7 起右轨写成 `minmax(0px, Npx)`（左轨仍是纯 `Npx`）：该列的当前
    // 宽度就是 minmax 的 max 端，取它；解析不了再回退调用方的 fallback。
    var m = /^(-?[\d.]+)px$/.exec(s)
    if (m !== null) return Number.parseFloat(m[1])
    var r = /minmax\(\s*(-?[\d.]+)px\s*,\s*(-?[\d.]+)px\s*\)/.exec(s)
    if (r !== null) return Number.parseFloat(r[2])
    return fallback
  }
  ...
  function solve() {
    ...
    // 右栏当前宽度：直接量面板。0.1.7 起第三轨是 minmax(0px, Npx)，pxOf 取 max 端
    // 已能解，但量面板更稳（格式再变也不怕）；面板不存在或不可见 = 0 = 没有右轨。
    var nativeRight = 0
    if (nodes.panel !== null && nodes.panel !== undefined) {
      try {
        var pw = nodes.panel.getBoundingClientRect().width
        if (pw > 0) nativeRight = Math.round(pw)
      } catch (e) { /* 量不到就退回 pxOf */ }
    }
    if (nativeRight === 0) nativeRight = pxOf(shell.right, 0)
    var collapsed = frame.hasAttribute('data-sidebar-collapsed')
    // 右栏是否真的在屏上：官方收起时 frame 上会落 data-rightbar-collapsed。
    // 只看面板宽度是不够的——0.1.7 的 dock 面板在收起后仍挂载（只是被平移到
    // frame 右边缘之外，README: docked content stays mounted while collapsed），
    // rect 仍有宽度。把那种状态当"有右轨"会把第三轨写成比例宽度，于是右栏视觉
    // 已收起、右侧却留下一大片空白（中列被活生生压扁）。
    var rightCollapsed = frame.hasAttribute('data-rightbar-collapsed')
    ...
    var right = null
    if (state.enabled && state.right !== null && nativeRight > 0 && !rightCollapsed) {
  ...
  function hijack(handle, axis) {
    ...
    handle.addEventListener('pointerdown', function (e) {
      ...
      // 该侧此刻可接管吗？不能接管就把这一次拖拽**还给官方**，绝不吞事件：
      //   - 右栏还没有独立轨道（用户想先把右栏拖出来）时我们写不出宽度，
      //     吞了事件 = 数字在跳、宽度不动，官方「拖出右栏」的路也被一起吃掉；
      //   - 左栏收起时同理，放行让官方把它展开。
      // solve() 只读 DOM 并更新认账快照，不写任何样式，在这里调用是安全的。
      var r = solve()
      if (r === null) return
      if ((axis === 'left' ? r.left : r.right) === null) return
      e.preventDefault()
      e.stopPropagation()
